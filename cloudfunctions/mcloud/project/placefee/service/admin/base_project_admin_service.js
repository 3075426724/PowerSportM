...
class BaseProjectAdminService extends BaseAdminService {
	...
	async isAdmin(token) { 
		...
		let admin = await AdminModel.getOne(where, 'ADMIN_ID,ADMIN_PHONE,ADMIN_NAME,ADMIN_TYPE,ADMIN_DESC,ADMIN_UNIT_ID');
		if (!admin)
	...
	async isSuperAdmin(token) {
		...
		let admin = await AdminModel.getOne(where, 'ADMIN_ID,ADMIN_PHONE,ADMIN_NAME,ADMIN_TYPE,ADMIN_UNIT_ID');
		if (!admin)
