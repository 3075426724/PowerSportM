...
class AdminEnrollService extends BaseProjectAdminService {
	...
	/** 管理员扫码核销 */
	async scanEnrollJoin(admin, code) {
		this.AppError('[多场馆]该功能暂不开放，如有需要请加作者微信：cclinux0730');
	}
	...
	async getAdminEnrollList({
		...
		let fields = 'ENROLL_UNIT_TITLE,ENROLL_DAYS,ENROLL_TITLE,ENROLL_CATE_ID,ENROLL_CATE_NAME,ENROLL_EDIT_TIME,ENROLL_ADD_TIME,ENROLL_ORDER,ENROLL_STATUS,ENROLL_VOUCH,ENROLL_EDIT_SET,ENROLL_CANCEL_SET,ENROLL_QR,ENROLL_OBJ';

		let where = {};
		where.and = {
			ENROLL_UNIT_ID: unitId,
	...
	// #####################导出登记数据
	/**获取登记数据 */
	async getEnrollJoinDataURL(unitId) {
		return await exportUtil.getExportDataURL(EXPORT_ENROLL_JOIN_DATA_KEY + '_' + unitId);
	}

	/**删除登记数据 */
	async deleteEnrollJoinDataExcel(unitId) {
		return await exportUtil.deleteDataExcel(EXPORT_ENROLL_JOIN_DATA_KEY + '_' + unitId);
	}
