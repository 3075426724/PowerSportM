...
class EnrollService extends BaseProjectService {
	...
	async minuteJob() {
		...
		for (let k in rows) {
			...
			if (!await this.fixEnrollJoinPay(tradeNo, enrollJoin.ENROLL_JOIN_UNIT_ID)) {
	...
	// 获得某场馆可用类型
	async getUnitCateList(unitId) {
		let cateList = await EnrollModel.distinct({ ENROLL_UNIT_ID: unitId }, 'ENROLL_CATE_ID');
		return cateList;
	}
	...
	// 取得某天内所有场地信息
	async getAllEnroll(userId, unitId, cateId, day) {
		let unit = await UnitModel.getOne({ UNIT_STATUS: 1, _id: unitId }, 'UNIT_TITLE');
		if (!unit) this.AppError('场馆不存在');


		if (userId) await this.fixUserEnrollJoinPayRecord(userId);

		let where = {
			ENROLL_UNIT_ID: String(unitId),
	...
	// 获取某天预订情况
	async getUsedByDay(unitId, cateId, day) {
		let where = {
			ENROLL_JOIN_UNIT_ID: String(unitId),
	...
	// 修正某订单状态 （仅需支付订单）
	async fixEnrollJoinPay(tradeNo, unitId) {
	...
	async enrollJoinEdit(userId, enrollId, enrollJoinId, forms) {
		...
		let unit = await UnitModel.getOne(enrollJoin.ENROLL_JOIN_UNIT_ID);
		if (!unit)
			this.AppError('该场馆不存在');
	...
	async detailForEnrollJoin(userId, enrollId, enrollJoinId = '') {
		...
		let fields = 'ENROLL_UNIT_TITLE,ENROLL_JOIN_FORMS, ENROLL_TITLE, ENROLL_CATE_NAME';
	...
	async cancelMyEnrollJoin(userId, enrollJoinId) {
		...
		this.statEnrollJoin(enrollJoin.ENROLL_JOIN_UNIT_ID);
