...
class AdminMgrController extends BaseProjectAdminController {
	...
	async delMgr() {
		...
		let service = new AdminMgrService();
		await service.delMgr(this._admin, input.id, this._adminId);
	...
	async getMgrList() {
		...
		let service = new AdminMgrService();
		let result = await service.getMgrList(this._admin, input);

		// 数据格式化
		let list = result.list;
		for (let k = 0; k < list.length; k++) {
			list[k].unitTitle = list[k].unit.UNIT_TITLE;
	...
	async getLogList() {
		...
		let service = new AdminMgrService();
		let result = await service.getLogList(this._admin, input);
