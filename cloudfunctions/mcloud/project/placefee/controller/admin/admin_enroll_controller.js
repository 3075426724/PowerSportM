...
class AdminEnrollController extends BaseProjectAdminController {
	...
	async getAdminAllEnroll() {
		...
		return await service.getAllEnroll('', input.unitId, input.cateId, input.day);
	}
	...
	async scanEnrollJoin() {
		...
		let service = new AdminEnrollService();
		return await service.scanEnrollJoin(this._admin, input.code);
	}
	...
	async insertEnroll() {
		...
		let rules = {
			...
			unitId: 'must|string|name=场馆',
			unitTitle: 'must|string|name=场馆名称',
	...
	async enrollJoinDataGet() {
		...
		if (input.isDel === 1)
			await service.deleteEnrollJoinDataExcel(input.unitId); //先删除

		return await service.getEnrollJoinDataURL(input.unitId);
	}
	...
	async enrollJoinDataDel() {
		...
		let service = new AdminEnrollService();
		return await service.deleteEnrollJoinDataExcel(input.unitId);
	}
	...
	async getTempList() {
		...
		let service = new AdminEnrollService();
		let result = await service.getTempList(input.unitId);
