...
class BaseProjectService extends BaseService {
	...
	async initSetup() {
		let F = (c) => 'bx_' + c;
		const INSTALL_CL = 'setup_companyhouse';
		...
		const MEET_TYPE = '1=预约量房';
		const ALBUM_CATE = '1=设计师';
		const PRODUCT_CATE = '1=设计方案';
			...
			if (albumCnt == 0) {
				...
				for (let j in albumArr) {
					...
					data.ALBUM_OBJ = {
						...
						level: '金牌',
						cover: [CONST_PIC],
						detail: [{ type: 'text', val: title + '内容1' }
						]
					};
